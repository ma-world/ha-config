...
This is a **public Home Assistant add-on repository**. The add-on uses the Home Assistant `/config` directory directly, creates a Git commit when changes are detected, and pushes it to the Git repository you configure. Everything runs inside Home Assistant.
...
3. It stores the selected `/config` files at the repository root and pushes changes to the configured branch.
...
The add-on backs up the mounted Home Assistant `/config` directory directly. External add-on data directories are not included by this version.
...
Open the **HA Config Sync** app page to change `sync_interval_hours`. To edit Git ignore rules, select **Open Web UI** on the same app page; saving there updates the local Git checkout immediately and is committed during the next sync without restarting the add-on. The Git Ignore Editor saves its rules directly as `/config/ha_config_sync.gitignore`. The add-on applies these rules before staging files from `/config`; changes take effect on the next sync. The Web UI also shows the most recent sync check, the timestamp of the latest commit in the local backup checkout, and an **Open private backup repository** button when a GitHub backup remote is configured. Use **Clear local Git cache** to discard the local clone and any unpushed local commits; the next sync downloads the backup repository again. The configured ignore rules apply to files from `/config` before they are committed.
